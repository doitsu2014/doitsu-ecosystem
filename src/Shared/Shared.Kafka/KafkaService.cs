...
namespace Shared.Kafka
{
    public class KafkaService : IKafkaService
    {
        ...
        public async Task<Either<Seq<ErrorString>, Unit>> DeleteTopicsAsync(params string[] topics)
        {
        ...
        public async Task<Either<Seq<ErrorString>, Unit>> CreateTopicsAsync(string[] topics, int numberOfPartition = 1, short replicationFactor = 1)
        {
            using var adminClient = new AdminClientBuilder(_settings.AdminClientConfig).Build();
            return await (from listTopics in StringValidator.ShouldNotNullOrEmpty(topics)
                    from nop in Success<ErrorString, int>(numberOfPartition)
                    from rf in Success<ErrorString, short>(replicationFactor)
